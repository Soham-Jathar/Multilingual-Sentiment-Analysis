...
export default function App() {
  ...
  return <main>
    ...
    <section className="workspace"><form className="analyzer" onSubmit={analyze}><div className="section-title"><div><p className="eyebrow">ANALYZE TEXT</p><h2>What are people saying?</h2></div><span className={`mode-pill ${mode}`}>{mode === 'fast' ? 'Baseline comparison' : 'XLM-RoBERTa'}</span></div><label htmlFor="feedback">Feedback text</label><textarea id="feedback" value={text} onChange={(event) => setText(event.target.value)} placeholder="Paste a review, comment, or message in any language..." maxLength="2000" /><div className="form-footer"><span>{text.length}/2000 characters</span><label className="switch translation-switch"><input type="checkbox" checked={translationEnabled} onChange={(event) => setTranslationEnabled(event.target.checked)} /><span className="slider" /><b>Translation</b></label>{translationEnabled && <label className="translate-select">Translate to<select value={targetLanguage} onChange={(event) => setTargetLanguage(event.target.value)}>{languages.map(([code, name]) => <option key={code} value={code}>{name}</option>)}</select></label>}<label className="switch"><input type="checkbox" checked={mode === 'transformer'} onChange={(event) => setMode(event.target.checked ? 'transformer' : 'fast')} /><span className="slider" /><b>Use XLM-RoBERTa</b></label><button disabled={loading}>{loading ? (translationEnabled ? 'Translating and analyzing...' : 'Analyzing...') : 'Analyze sentiment'} <span>-&gt;</span></button></div>{error && <p className="error" role="alert">{error}</p>}</form>{result ? <Result result={result} /> : <section className="empty-result"><div className="empty-mark">*</div><p className="eyebrow">READY WHEN YOU ARE</p><h2>Your insight will appear here.</h2><p>Enter feedback above to identify its language and sentiment.</p></section>}</section>
  </main>
}
